...
# --- the parity-skip count ----------------------------------------------

# "seven" -> 7. Prints "?" for anything it does not know, so an unrecognised
# word fails loudly rather than comparing as equal to nothing.
word_to_number() {
  case "$1" in
...
@test "the number of parity skips the docs promise is the number there are" {
  # Counted from the docs' side, not mapped to it: an earlier version of this
  # case turned the real number into a word with `sed -n "$((real+1))p"` over a
  # list with a leading empty element, which is off by one - it compared
  # "seven" against a real count of 8 and passed by coincidence. Reading the
  # word the docs use and converting it to a number has no index arithmetic to
  # get wrong.
  local real
  real="$(mise exec -- python3 -c "
import re, glob, os
root = os.environ['REPO_ROOT']
n = 0
for f in sorted(glob.glob(os.path.join(root, 'test/*.bats'))):
    # This file is excluded on purpose: its own case names parity_skip in order
    # to count it, and a census that counts the counter is wrong by one.
    if os.path.basename(f) == 'docs-facts.bats':
        continue
    blocks = re.split(r'^@test ', open(f).read(), flags=re.M)[1:]
    n += sum(1 for b in blocks if 'parity_skip' in b)
print(n)
")"
  [ "$real" -gt 0 ] || fail "no parity_skip cases found - has the helper been renamed?"

  local claimed_agents claimed_contrib
  claimed_agents="$(grep -oE 'Without them [a-z]+ cases skip' "$REPO_ROOT/AGENTS.md" | awk '{print $3}')"
  claimed_contrib="$(grep -oE 'and the [a-z]+ skips you will see' "$REPO_ROOT/CONTRIBUTING.md" | awk '{print $3}')"
  [ -n "$claimed_agents" ] || fail "AGENTS.md no longer states how many cases skip"
  [ -n "$claimed_contrib" ] || fail "CONTRIBUTING.md no longer states how many skips to expect"

  local n_agents n_contrib
  n_agents="$(word_to_number "$claimed_agents")"
  n_contrib="$(word_to_number "$claimed_contrib")"
  [ "$n_agents" = "$real" ] || fail "AGENTS.md says $claimed_agents ($n_agents) cases skip; $real do"
  [ "$n_contrib" = "$real" ] || fail "CONTRIBUTING.md says $claimed_contrib ($n_contrib) skips; $real do"
}

@test "the parity census excludes itself, and the word conversion is exact" {
  # The two bugs the case above had, pinned so they cannot come back.
  [ "$(word_to_number seven)" = "7" ] || fail "seven is not 7: $(word_to_number seven)"
  [ "$(word_to_number four)" = "4" ] || fail "four is not 4: $(word_to_number four)"
  [ "$(word_to_number nonsense)" = "?" ] || fail "an unknown word must not resolve to a number"
  grep -q "docs-facts.bats" "$BATS_TEST_FILENAME" || fail "the census no longer excludes this file"
}
